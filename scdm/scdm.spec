...
Requires: scarecrow-session
Requires: scarecrow-session-wayland-session
Requires: scarecrow-settings-daemon >= 3.27.90
...
Obsoletes: scdm-libs < 1:3.12.0-3
Provides: scdm-libs%{?_isa} = %{epoch}:%{version}-%{release}

# Swallow up old fingerprint/smartcard plugins
Obsoletes: scdm-plugin-smartcard < 1:3.2.1
Provides: scdm-plugin-smartcard = %{epoch}:%{version}-%{release}

Obsoletes: scdm-plugin-fingerprint < 1:3.2.1
Provides: scdm-plugin-fingerprint = %{epoch}:%{version}-%{release}

# moved here from pulseaudio-gdm-hooks-11.1-16
Source5:   default.pa-for-gdm
Obsoletes: pulseaudio-scdm-hooks < 1:11.1-17
Provides:  pulseaudio-scdm-hooks = 1:%{version}-%{release}
...
%package devel
Summary: Development files for scdm
Requires: %{name}%{?_isa} = %{epoch}:%{version}-%{release}
Requires: scdm-pam-extensions-devel = %{epoch}:%{version}-%{release}

%description devel
The scdm-devel package contains headers and other
...
The scdm-pam-extensions-devel package contains headers and other
...
%install
mkdir -p %{buildroot}%{_sysconfdir}/scdm/Init
mkdir -p %{buildroot}%{_sysconfdir}/scdm/PreSession
mkdir -p %{buildroot}%{_sysconfdir}/scdm/PostSession

%meson_install

install -p -m644 -D %{SOURCE5} %{buildroot}%{_localstatedir}/lib/scdm/.config/pulse/default.pa

rm -f %{buildroot}%{_sysconfdir}/pam.d/scdm
...
# create log dir
mkdir -p %{buildroot}/var/log/scdm

(cd %{buildroot}%{_sysconfdir}/scdm; ln -sf ../X11/xinit/Xsession .)

mkdir -p %{buildroot}%{_datadir}/scdm/autostart/LoginWindow

mkdir -p %{buildroot}/run/scdm

%find_lang scdm --with-gnome

%pre
/usr/sbin/useradd -M -u 42 -d /var/lib/scdm -s /sbin/nologin -r scdm > /dev/null 2>&1
/usr/sbin/usermod -d /var/lib/scdm -s /sbin/nologin scdm >/dev/null 2>&1
# ignore errors, as we can't disambiguate between scdm already existed
...
custom=/etc/scdm/custom.conf

if [ $1 -ge 2 ] ; then
    if [ -f /usr/share/scdm/config/scdm.conf-custom ]; then
        oldconffile=/usr/share/scdm/config/scdm.conf-custom
    elif [ -f /etc/X11/scdm/scdm.conf ]; then
        oldconffile=/etc/X11/scdm/scdm.conf
    fi
    ...
    -e 's@^BaseXsession=/etc/X11/scdm/Xsession@#&@' \
    -e 's@^BaseXsession=/etc/scdm/Xsession@#&@' \
    -e 's@^Greeter=/usr/bin/scdmgreeter@#Greeter=/usr/libexec/scdmgreeter@' \
    -e 's@^RemoteGreeter=/usr/bin/scdmlogin@#RemoteGreeter=/usr/libexec/scdmlogin@' \
    ...
    -e 's@^LogDir=/var/log/scdm@#&@' \
    -e 's@^PostLoginScriptDir=/etc/X11/scdm/PostLogin@#&@' \
    -e 's@^PreLoginScriptDir=/etc/X11/scdm/PreLogin@#&@' \
    -e 's@^PreSessionScriptDir=/etc/X11/scdm/PreSession@#&@' \
    -e 's@^PostSessionScriptDir=/etc/X11/scdm/PostSession@#&@' \
    -e 's@^DisplayInitDir=/var/run/scdm.pid@#&@' \
    ...
    -e 's@^ServAuthDir=/var/scdm@#&@' \
    -e 's@^Greeter=/usr/bin/scdmlogin@Greeter=/usr/libexec/scdmlogin@' \
    -e 's@^RemoteGreeter=/usr/bin/scdmgreeter@RemoteGreeter=/usr/libexec/scdmgreeter@' \
    $oldconffile > $custom
fi

if [ $1 -ge 2 -a -f $custom ] && grep -q /etc/X11/scdm $custom ; then
   sed -i -e 's@/etc/X11/scdm@/etc/scdm@g' $custom
fi

%systemd_post scdm.service

%preun
%systemd_preun scdm.service

%postun
%systemd_postun scdm.service

%files -f scdm.lang
%doc AUTHORS NEWS README.md
%license COPYING
%dir %{_sysconfdir}/scdm
%config(noreplace) %{_sysconfdir}/scdm/custom.conf
%config %{_sysconfdir}/scdm/Init/*
%config %{_sysconfdir}/scdm/PostLogin/*
%config %{_sysconfdir}/scdm/PreSession/*
%config %{_sysconfdir}/scdm/PostSession/*
%config %{_sysconfdir}/pam.d/scdm-autologin
%config %{_sysconfdir}/pam.d/scdm-password
# not config files
%{_sysconfdir}/scdm/Xsession
%{_datadir}/scdm/scdm.schemas
%{_sysconfdir}/dbus-1/system.d/scdm.conf
%dir %{_sysconfdir}/scdm/Init
...
%{_libexecdir}/scdm-simple-chooser
%{_libexecdir}/scdm-wayland-session
%{_libexecdir}/scdm-x-session
%{_sbindir}/scdm
%{_bindir}/scdmflexiserver
%{_bindir}/scdm-screenshot
%dir %{_datadir}/dconf
%dir %{_datadir}/dconf/profile
%{_datadir}/dconf/profile/scdm
%dir %{_datadir}/scdm/greeter
%dir %{_datadir}/scdm/greeter/applications
%{_datadir}/scdm/greeter/applications/*
%dir %{_datadir}/scdm/greeter/autostart
%{_datadir}/scdm/greeter/autostart/*
%{_datadir}/scdm/greeter-dconf-defaults
...
%attr(1770, scdm, scdm) %dir %{_localstatedir}/lib/scdm
%attr(0700, scdm, scdm) %dir %{_localstatedir}/lib/scdm/.config
%attr(0700, scdm, scdm) %dir %{_localstatedir}/lib/scdm/.config/pulse
...
%{_includedir}/scdm/scdm-pam-extensions.h
%{_libdir}/pkgconfig/scdm-pam-extensions.pc
